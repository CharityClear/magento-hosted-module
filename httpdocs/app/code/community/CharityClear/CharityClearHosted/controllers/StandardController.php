...
class CharityClear_CharityClearHosted_StandardController extends Mage_Core_Controller_Front_Action {

    /**
    * Get singleton with CharityclearHosted standard order transaction information
    *
    * @return Charityclear_CharityclearHosted_Model_Standard
    */
    public function getStandard() {
    
        return Mage::getSingleton('CharityClearHosted/standard');
        
    }

    /**
    * When a customer chooses Charityclear on Checkout/Payment page
    *
    */
    public function redirectAction() {
    
        $this->getResponse()->setBody($this->getLayout()->createBlock('CharityClearHosted/standard_redirect')->toHtml());
    ...
    */
    public function successAction() {
        ...
        $collection = Mage::getModel('CharityClearHosted/CharityClearHosted_Trans')->getCollection();
        ...
        if( $transid ){
            ...
            $trn = Mage::getModel('CharityClearHosted/CharityClearHosted_Trans')->loadById( $trans['id'] );
            ...
            if( $_POST['responseCode'] === "0") {
                ...
                if( $_POST['amountReceived'] == $trans['amount'] ){
                    ...
                    if( $order->getId() ){
                        ...
                        if( !$order->canInvoice() ) {
                            ...
                            $order->addStatusToHistory( Mage::getStoreConfig('payment/CharityClearHosted_standard/successfulpaymentstatus'), $this->buildordermessage(), 0);
                            $order->save();

                            //when order cannot create invoice, need to have some logic to take care
                            $order->addStatusToHistory(
                                        $order->getStatus(),
                                        Mage::helper('CharityClearHosted')->__('Order cannot create invoice')
                            );
                    ...
                    if( $order->getId() ){
                        ...
                        //Add order comment and update status
                        $order->addStatusToHistory( Mage::getStoreConfig('payment/CharityClearHosted_standard/order_status'), $this->buildordermessage(), 0);
                        $order->addStatusToHistory( Mage::getStoreConfig('payment/CharityClearHosted_standard/order_status'), "The amount paid did not match the amount due.", 0);
                ...
                if( $order->getId() ){
                    ...
                    //Add order comment and update status.
                    $order->addStatusToHistory( Mage::getStoreConfig('payment/CharityClearHosted_standard/unsuccessfulpaymentstatus'), $this->buildordermessage(), 0);
                    $order->save(); 
                ...
                $block = $this->getLayout()->createBlock(
                    ...
                    'CharityClearHosted/standard_failure',
                    array('template' => 'CharityClearHosted/standard/failure.phtml')
                );
            ...
            $order = Mage::getModel('sales/order');
            $order->loadByIncrementId( Mage::getSingleton('checkout/session')->getCharityclearHostedOrderId() );

            if( $order->getId() ){

                //Add order comment and update status.
                $order->addStatusToHistory( Mage::getStoreConfig('payment/CharityClearHosted_standard/order_status'), $this->buildordermessage(), 0);
                $order->addStatusToHistory( Mage::getStoreConfig('payment/CharityClearHosted_standard/order_status'), "Unable to locate the transaction in the CharityClearHosted table", 0);
                $order->save(); 

            }
            
            $session = Mage::getSingleton('checkout/session');
            $session->setQuoteId( Mage::getSingleton('checkout/session')->getCharityclearHostedQuoteId() );
            Mage::getModel('sales/quote')->load($session->getQuoteId())->setIsActive(true)->save();
    ...
    function buildordermessage() {
        ...
        $ordermessage = "CharityClear Payment<br /><br />" . $paymentoutcome . "<br /><br />Amount Received: " . $amountreceived . "<br />Message: \"" . ucfirst( $_POST['responseMessage'] ) . "\"<br />Xref: " . $_POST['xref'];
